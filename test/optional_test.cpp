...
bool throws_exc(std::function<void() > fun) {
    try {
        fun();
    } catch (const sl::support::bad_optional_access_exception& e) {
        (void) e;
        return true;
    }
    return false;
...
void test_some() {
    auto some = sl::support::make_optional(42);
...
void test_none() {
    auto none = sl::support::optional<int>();
...
void test_nocopy() {
    auto some = sl::support::make_optional(std::unique_ptr<std::string>(new std::string("42")));
    slassert(some.has_value());
    slassert(some);
    slassert("42" == *some.value());
    // will be moved out
